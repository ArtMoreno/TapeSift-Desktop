...
Initial native sync probes timed out while synchronous QTest polling prevented the Python worker from running normally. A minimal thread experiment reproduced this in the harness; using a nested Qt event loop allowed normal worker execution and completion on both platforms, with no production workaround. The failed attempts are retained. These checks use local folder fixtures and do not verify a cloud provider's network transfer, Wayland interaction for this dialog, or the full regression suite.
